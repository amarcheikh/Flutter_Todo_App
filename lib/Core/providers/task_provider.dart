...
class TaskProvider extends ChangeNotifier {
  ...
  TaskFilter currentFilter = TaskFilter.tout;
  TaskSortOption currentOption = TaskSortOption.aucun;
  ...
  void updateTaskStatus(String taskId, bool newStatus) {
    _firestore
        .collection('tasks')
        .doc(taskId)
        .update({'termine': newStatus}).then((value) {
  ...
  void fetchTasks() {
    ...
    switch (currentFilter) {
      case TaskFilter.termine:
        query = _firestore
            .collection('tasks')
            .where('termine', isEqualTo: true)
            .where('uid', isEqualTo: currentUserUid);
        break;
      case TaskFilter.encour:
        query = _firestore
            .collection('tasks')
    ...
    if (currentOption == TaskSortOption.datefin) {
      query = _firestore.collection('tasks').orderBy('fin');
    } else if (currentOption == TaskSortOption.datedebut) {
      query = _firestore.collection('tasks').orderBy('debut');
    }
